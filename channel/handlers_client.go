...
func (server Server) playerUseMysticDoor(conn mnet.Client, reader mpacket.Reader) {
	...
	// Use the actual portal that already exists in the destination instance
	if destPortalIdx >= 0 && destPortalIdx < len(dstInst.portals) {
		...
		log.Printf("[Mystic Door] Portal position: (%d, %d), destFieldID: %d, name: %s", 
			dstPortal.pos.x, dstPortal.pos.y, dstPortal.destFieldID, dstPortal.name)
		
		// Warp directly to the existing portal (whether it's the source door or town "tp" portal)
		...
		log.Printf("[Mystic Door] SUCCESS: Player %d warped to map %d at (%d, %d)", plr.ID, destMapID, dstPortal.pos.x, dstPortal.pos.y)
	} else {
