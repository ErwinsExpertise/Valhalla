...
func (p *Player) removeActiveSummonForSkill(skillID int32, reason byte) {
	...
	case skill.Puppet, skill.SniperPuppet:
		found := p.summons != nil && p.summons.puppet != nil
		if found {
			p.broadcastRemoveSummon(p.summons.puppet.SkillID, reason)
			p.summons.puppet = nil
			return
		}

	case skill.SummonDragon, skill.SilverHawk, skill.GoldenEagle:
		found := p.summons != nil && p.summons.summon != nil
		if found {
			p.broadcastRemoveSummon(p.summons.summon.SkillID, reason)
			p.summons.summon = nil
			return
		}
	}
